...
namespace ZONEDOCTOR
{
    public partial class GraphicEditor : NewForm
    {
        ...
        private void ResetToolstrip(ToolStripItem except)
        {
            if (except != subtileDraw) subtileDraw.Checked = false;
            if (except != subtileErase) subtileErase.Checked = false;
            if (except != subtileDropper) subtileDropper.Checked = false;
            if (except != subtileReplaceColor) subtileReplaceColor.Checked = false;
            if (except != subtileFill) subtileFill.Checked = false;
            if (except != subtileSelect) subtileSelect.Checked = false;
            if (except != graphicZoomIn) graphicZoomIn.Checked = false;
            if (except != graphicZoomOut) graphicZoomOut.Checked = false;
            syncTileset.Visible = subtileSelect.Checked;
            contiguous.Visible = subtileFill.Checked;
            brushSize.Visible = subtileDraw.Checked || subtileErase.Checked;
        }
        private void UpdateSubtiles()
        {
            if (!syncTileset.Checked || oldsubtiles == null)
                return;
            int[] newsubtiles = GetSubtiles();
            Locations locations = (Locations)update.Target;
            Tileset tileset = locations.Tileset;
            foreach (Tile[] tiles in tileset.Tilesets_tiles)
            {
                if (tiles == null)
                    continue;
                foreach (Tile tile in tiles)
                {
                    ...
                    {
                        int index = Array.IndexOf(oldsubtiles, subtile.Index);
                        if (index == -1)
                            continue;
                        subtile.Index = newsubtiles[index];
                    }
                }
            }
            oldsubtiles = null;
        }
        private int[] GetSubtiles()
        {
            int i = 0;
            int[] dst = new int[(overlay.Select.Width / 8) * (overlay.Select.Height / 8)];
            for (int y = overlay.Select.Y / 8; y < overlay.Select.Terminal.Y / 8; y++)
            {
                for (int x = overlay.Select.X / 8; x < overlay.Select.Terminal.X / 8; x++)
                {
                    if (i >= dst.Length)
                        continue;
                    dst[i++] = y * size.Width + x;
                }
            }
            return dst;
        }
        ...
        private void pictureBoxGraphicSet_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            else if (action == Drawing.Erase)
            {
                ...
                if (!showBG.Checked)
                    ...
                    pictureBoxGraphicSet.Draw(hoverBox, Color.Black);
            }
            ...
            else if (subtileSelect.Checked)
            {
                ...
                if (e.Button == MouseButtons.Left && mouseDownObject == null && overlay.Select != null)
                {
                    ...
                    overlay.Select.Final = new Point(
                        Math.Max(0, Math.Min(x / zoom, this.width / zoom)),
                        Math.Max(0, Math.Min(y / zoom, this.height / zoom)));
                }
        ...
        private void exportToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("There was a problem exporting the graphic block.", "ZONE DOCTOR");
            }
        }
        ...
        {
            ResetToolstrip(subtileReplaceColor);
            //
        ...
        private void syncTileset_VisibleChanged(object sender, EventArgs e)
        {
            toolStripSeparator33.Visible = syncTileset.Visible;
        }
